...
function seleccionarDayamon(mascotaJugador) {
    fetch(`http://192.168.100.15:8080/dayamon/${jugadorId}`, {
        method: "post",
        headers: {
...
function enviarAtaques(){
    fetch(`http://192.168.100.15:8080/dayamon/${jugadorId}/ataques`, {
        method: "post",
        headers: {
            "Content-Type": "application/json"
        },
...
function obtenerAtaques() {
    fetch(`http://http://192.168.100.15:8080/dayamon/${enemigoId}/ataques`)
        .then(function(res){
            if (res.ok){
                res.json()
...
dayamonesEnemigos.forEach(function(dayamon){
    dayamon.pintarDayamon()
    revisarColision(dayamon)
})
   
}

function enviarPosicion(x, y) {
    fetch(`http://192.168.100.15:8080/dayamon/${jugadorId}/posicion`, {
        method: "post",
        headers: {
            "Content-Type": "application/json"
        },
        ...
        if (res.ok) {
            res.json()
                .then(function ({ enemigos }) {
                    ...
                    dayamonesEnemigos=enemigos.map(function (enemigo) {
                        let dayamonEnemigo = null
                        const dayamonNombre = enemigo.dayamon.nombre || ""
                        if (dayamonNombre === "Squirtle") {
                            dayamonEnemigo = new Dayamon('Squirtle', 'https://64.media.tumblr.com/ca7bd49d1d414f1ccdaa4950094b6754/tumblr_n28v8gQLkH1rcgdmto3_250.gifv', 5, './assets/Squirtle.png',enemigo.id)
                        } else if (dayamonNombre === "Charmander") {
                            dayamonEnemigo = new Dayamon('Charmander', 'https://64.media.tumblr.com/6035bf1768e0c05901193ac1129df39d/tumblr_n28v8gQLkH1rcgdmto2_250.gifv', 5, './assets/Charmander.png',enemigo.id)
                        } else if (dayamonNombre === "Bulbasaur") {
                            dayamonEnemigo = new Dayamon('Bulbasaur', 'https://64.media.tumblr.com/4a724672ff10c9886c94434a00954a99/tumblr_n28v8gQLkH1rcgdmto1_250.gifv', 5, './assets/Bulbasaur.png',enemigo.id)
                        }

                        dayamonEnemigo.x = enemigo.x
                        dayamonEnemigo.y = enemigo.y

                        return dayamonEnemigo
                    })
                })
        }
    })
